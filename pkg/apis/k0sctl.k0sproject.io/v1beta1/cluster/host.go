...
func (h *Host) DrainNode(node *Host, options DrainOption) error {
	return h.Sudo().Exec(h.Configurer.KubectlCmdf(h, h.K0sDataDir(), "drain %s %s", options.ToKubectlArgs(h.FS()), quote(h.FS(), node.Metadata.Hostname)))
}

// CordonNode marks the node unschedulable
func (h *Host) CordonNode(node *Host) error {
	return h.Sudo().Exec(h.Configurer.KubectlCmdf(h, h.K0sDataDir(), "cordon %s", quote(h.FS(), node.Metadata.Hostname)))
}

// UncordonNode marks the node schedulable
func (h *Host) UncordonNode(node *Host) error {
	return h.Sudo().Exec(h.Configurer.KubectlCmdf(h, h.K0sDataDir(), "uncordon %s", quote(h.FS(), node.Metadata.Hostname)))
}

// DeleteNode deletes the given node from kubernetes
func (h *Host) DeleteNode(node *Host) error {
	return h.Sudo().Exec(h.Configurer.KubectlCmdf(h, h.K0sDataDir(), "delete node %s", quote(h.FS(), node.Metadata.Hostname)))
}

// Taints returns all taints added to the node.
func (h *Host) Taints(node *Host) ([]string, error) {
	output, err := h.Sudo().ExecOutput(h.Configurer.KubectlCmdf(h, h.K0sDataDir(), `get node %s -o jsonpath='{range .spec.taints[*]}{.key}={.value}:{.effect}{"\n"}{end}'`, quote(h.FS(), node.Metadata.Hostname)))
	if err != nil {
...
func (h *Host) AddTaint(node *Host, taint string) error {
	return h.Sudo().Exec(h.Configurer.KubectlCmdf(h, h.K0sDataDir(), "taint nodes --overwrite %s %s", quote(h.FS(), node.Metadata.Hostname), quote(h.FS(), taint)))
}
...
func (h *Host) RemoveTaint(node *Host, taint string) error {
	...
	return h.Sudo().Exec(h.Configurer.KubectlCmdf(h, h.K0sDataDir(), "taint nodes %s %s-", quote(h.FS(), node.Metadata.Hostname), quote(h.FS(), taint)))
}
