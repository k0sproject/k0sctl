...
func (p *InstallControllers) Run() error {
	...
	if p.numRunning == 1 && len(remaining) >= 2 {
		...
		if err := firstBatch.BatchedParallelEach(perBatch, p.installK0s); err != nil {
			return err
		}
	...
	if p.numRunning%2 == 0 {
		...
		if err := p.installK0s(remaining[0]); err != nil {
			return err
		}
	...
	// install the rest in parallel in quorum-optimized batches
	for len(remaining) > 0 {
		...
		if err := batch.BatchedParallelEach(perBatch, p.installK0s); err != nil {
			return err
		}
...
func (p *InstallControllers) installK0s(h *cluster.Host) error {
	...
	err = p.Wet(h, fmt.Sprintf("install k0s controller using `%s", strings.ReplaceAll(cmd, h.Configurer.K0sBinaryPath(), "k0s")), func() error {
		...
		if err := retry.Timeout(context.Background(), retry.DefaultTimeout, node.ServiceRunningFunc(h, h.K0sServiceName())); err != nil {
			return err
		}

		err = retry.Timeout(context.Background(), retry.DefaultTimeout, func(_ context.Context) error {
